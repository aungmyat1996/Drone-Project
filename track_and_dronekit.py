...
# Initialize YOLOv8s model for person/car detection
model = YOLO('yolov8s.pt')

# Connect to the drone
# Replace '127.0.0.1:14550' with your drone connection string
...
AUTO_TRACKING = False  # Flag to switch between manual and auto-tracking modes
DRONE_SPEED = 5  # Speed for automatic tracking
...
def draw_corner_brackets(frame):
    """ Draw smaller corner brackets on the frame. """
    h, w = frame.shape[:2]
    bracket_len = int(min(w, h) * 0.07)  # 3% of the shortest dimension for smaller size
    thickness = 2
    offset = int(min(w, h) * 0.3)  # 5% offset from the edges
    # Top-left corner
...
def update_trackers(frame):
    ...
    for tracker in trackers:
        ...
        if success:
            ...
            # Send drone movement commands based on object position
            if AUTO_TRACKING:
                move_drone_based_on_position(car_center_x, car_center_y, frame.shape[1] // 2, frame.shape[0] // 2)
...
def move_drone_based_on_position(obj_x, obj_y, frame_center_x, frame_center_y):
    """ Move the drone based on object position in the frame. """
    if obj_x < frame_center_x - 50:  # Object is to the left of the center
        vehicle.channels.overrides['yaw'] = 1500 + DRONE_SPEED
    elif obj_x > frame_center_x + 50:  # Object is to the right of the center
        vehicle.channels.overrides['yaw'] = 1500 - DRONE_SPEED
    else:
        vehicle.channels.overrides['yaw'] = 1500  # Keep the yaw steady if the object is centered
    if obj_y < frame_center_y - 50:  # Object is above the center
        vehicle.channels.overrides['pitch'] = 1500 - DRONE_SPEED
    elif obj_y > frame_center_y + 50:  # Object is below the center
        vehicle.channels.overrides['pitch'] = 1500 + DRONE_SPEED
    else:
        vehicle.channels.overrides['pitch'] = 1500  # Keep the pitch steady if the object is centered
...
def main():
    ...
    while True:
        ...
        cv2.imshow('YOLOv8 Car Tracking with Drone Control', frame)

        # Check for mode switch (toggle between manual and auto-tracking)
